...
for model in "${models[@]}"; do
    ...
    if command -v /usr/bin/time &> /dev/null; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ...
            /usr/bin/time -l node packages/lib/out/index.js "$input_file" > /dev/null 2> "$temp_stderr"
            make_exit_code=$?
            ...
            /usr/bin/time -o "$temp_stderr" -f "%M" node packages/lib/out/index.js "$input_file" > /dev/null 2>&1
            make_exit_code=$?
        ...
        node packages/lib/out/index.js "$input_file" > /dev/null 2>&1
        make_exit_code=$?
