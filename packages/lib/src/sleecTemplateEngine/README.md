...
The SLEEC template engine has evolved through several key improvements:

### Initial Implementation (commit `adebf22`)

**Features:**
- Basic rule generation
- Hardcoded measure types
- Simple avoid event handling

**Limitations:**
- No measure extraction
- No event classification
- Fixed scale values: `scale(low, moderate, high)`

### Definition Block Addition (commit `deb1d79`)

**Added:**
- `def_start...def_end` block generation
- Event extraction and classification
- External vs. fluent event separation
- Measure type detection (boolean vs. scale)

**Impact:** Generated valid SLEEC specifications with proper structure

### Transformation Improvements (commit `1930cab`)

**Enhanced:**
- Fluent name extraction from task names
- Event grouping and formatting
- Proper spacing and indentation

**Fixed:**
- Fluent event generation logic
- Event deduplication

### Rule Fixes (commits `e95e0ea`, `dc8a15a`)

**Improved:**
- Rule syntax formatting
- Avoid event rule generation
- Temporal constraint handling

**Fixed:**
- Missing `unless` clauses
- Incorrect fluent state transitions

### ObstacleEvent Refactoring (commits `e204f40`, `d265f90`, `b79a993`)

**Changed:**
- Renamed `AvoidEvent` → `ObstacleEvent` (clearer semantics)
- Updated function names for consistency
- Improved obstacle rule naming: `Rule4` → `RuleObstacle`

**Impact:** More intuitive API and better code readability

### Scale Measure Enhancement (commit `b17e8f5`, PR #5)

**Major Feature:** Automatic scale value extraction and semantic ordering

**Added:**
- `scaleValues?: string[]` field to `Measure` type
- `semanticSortScaleValues()` function for intelligent ordering
- Value extraction from condition expressions
- Pattern-based tier detection (low/medium/high)

**Before:**
```sleec
measure patientDiscomfort: scale(low, moderate, high)  // Hardcoded
```

**After:**
```sleec
measure patientDiscomfort: scale(d_low, d_moderate, d_high)  // Extracted from actual usage
```

**Benefits:**
- Declarations match actual usage in rules
- Supports arbitrary scale value names
- Maintains semantic order regardless of input order
- No manual configuration needed

### Current State

**Capabilities:**
- ✅ Automatic event extraction and classification
- ✅ Smart measure detection (boolean vs. scale)
- ✅ Semantic scale value ordering
- ✅ Obstacle event support
- ✅ Temporal constraint handling
- ✅ Proper SLEEC syntax generation
- ✅ Task lifecycle rule generation
- ✅ Fluent-based state management

**Validation:**
- 14 comprehensive tests for measure extraction
- Semantic ordering tests for various patterns
- Integration tests with real BSN goal model
