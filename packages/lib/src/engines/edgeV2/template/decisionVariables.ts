...
export const decisionVariablesForGoal = ({
  ...
  const decisionSpaces = goal.properties.engine.decision.decisionVars.map(
    (decision: { variable: string; space: number }) =>
      Array.from({ length: decision.space }, (_, i) => i),
  );
  const variableNames = goal.properties.engine.decision.decisionVars.map(
...
const hasDecisionSpace = (goal: EdgeGoalNode): boolean => {
  const hasDecisionVars = goal.properties.engine.decision.decisionVars.length > 0;
