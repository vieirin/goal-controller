...
const parseVariable = (line: string): VariableInfo | null => {
  // Match patterns like:
  // G0_pursued : [0..1] init 0;
  // variable: bool init true;
  // variable: [0..10] init 5;
  const varMatch = line.match(
    /^\s*(\w+)\s*:\s*(?:\[(\d+)\.\.(\d+)\]|bool)\s*init\s*(\w+)\s*;/,
  );
 ...
 */
const extractVariablesFromGuard = (guard: string): string[] => {
...
const parseTransition = (line: string): TransitionInfo | null => {
  // Match patterns like:
  ...
  if (!label || !guard || !update) return null;
...
const parseModule = (
  lines: string[],
  startIndex: number,
): { module: ModuleInfo; nextIndex: number } | null => {
